...
exports.createUser = async (req, res) => {
    try {
        ...
        if (!surname || !lastname || !email || !password || !phone || !role || !address) {
            return res.status(400).json({
                success: false,
    ...
    try {
        ...
        const formattedStudents = students.map(s => ({
            ...
            parentName: s.parentId
                ? `${s.parentId.title || ""} ${s.parentId.surname || ""} ${s.parentId.middlename || ""} ${s.parentId.lastname || ""}`
                    .replace(/\s+/g, " ")
                    .trim()
                : "N/A",
...
exports.updateTeacher = async (req, res) => {
    try {
        ...
        teacher.subjects = subjects.split(",").map(s => sx.trim());
...
exports.createTeacher = async (req, res) => {
    try {
        ...
        if (!subjects || subjects.length === 0) {
            return res.status(400).json({ message: "Subjects is required" })
        }
        if (!specialization) {
        ...
        if (!user || user.role !== "Teacher") {
            return res.status(400).json({ message: "Invaild teacher user selected" })
        }
...
exports.getTeacherUsers = async (req, res) => {
    try {
        ...
        const formatted = users.map(user => ({
            id: user._id,
            fullName: `${user.surname} ${user.middlename || ""} ${user.lastname}`.replace(/\s+/g, " ").trim(), staffNo: user.staffNo
        }));
...
exports.getAcademicAnalytics = async (req, res) => {
    try {
        ...
        console.error("Academic analytic error", error);
...
exports.getAllSubjects = async (req, res) => {
    try {
        ...
        const formattedSubjects = subjects.map((s) => {
            const teacherUser = s.teacherId?.userId;
            ...
            return {
                ...
                teacherName, // ✅ unified field name
                createdAt: s.createdAt,
            };
        });
...
exports.getAllAttendance = async (req, res) => {
    try {
        ...
        const formattedAttendance = attendance.map((p) => ({
            id: p._id,

            studentName: p.studentId
                ? `${p.studentId.userId.title || ""} ${p.studentId.userId.surname} ${p.studentId.userId.middlename || ""} ${p.studentId.userId.lastname}`
                    .replace(/\s+/g, " ")
                    .trim()
                : "N/A",
            className: p.klassId?.name || "N/A",
            termName: p.termId?.name || "N/A",
            sessionName: p.sessionId?.name || "N/A",

            markedByName: p.markedBy?.userId
                ? `${p.markedBy.userId.title || ""}${p.markedBy.userId.surname} ${p.markedBy.userId.middlename || ""} ${p.markedBy.userId.lastname}`
                    .replace(/\s+/g, " ")
                    .trim()
                : "N/A",
            data: p.date,
        ...
        res.status(200).json({
            ...
            data: {
                ...
                studentName: attendance.studentId
                    ? `${attendance.studentId.userId.title || ""} ${attendance.studentId.userId.surname} ${attendance.studentId.userId.middlename || ""} ${attendance.studentId.userId.lastname}`
                        .replace(/\s+/g, " ")
                        .trim()
                    : "N/A",
                ...
                markedByName: attendance.markedBy?.userId
                    ? `${attendance.markedBy.userId.title || ""} ${attendance.markedBy.userId.surname} ${attendance.markedBy.userId.middlename || ""} ${attendance.markedBy.userId.lastname}`
                        .replace(/\s+/g, " ")
                        .trim()
                    : "N/A",
...
exports.getAttendanceFormData = async (req, res) => {
    try {
        ...
        const terms = await Term.find().select("_id name title"); // 👈 support both

        const formatName = (u) => {
            if (!u) return "No Name";
            return `${u.title || ""} ${u.surname || ""} ${u.middlename || ""} ${u.lastname || ""}`
                .replace(/\s+/g, " ")
                .trim();
        };
        ...
        const response = {
            ...
            terms: terms.map((t) => ({
                value: t._id,
                label: t.name || t.title || "Unnamed Term",
            })),
        ...
        console.log("FORM DATA SENT:", response); // 🔥 debug
...
exports.updateTest = async (req, res) => {
    try {
        ...
        const { title, description, subjectId, klassId, sessionId, termId, duration, status, questions } = req.body;
        ...
        console.error("Updated test error:", error);
...
exports.getTestFormData = async (req, res) => {
    try {
        ...
        const formatName = (u) => `${u?.surname} ${u?.lastname}`;
...
exports.createTest = async (req, res) => {
    try {
        const { title, description, subjectId, klassId, sessionId, termId, created_by, questions, duration, status } = req.body;

        if (!title || !subjectId || !klassId || !sessionId || !termId || !created_by) {
            return res.status(400).json({
...
exports.getQuizById = async (req, res) => {
    try {
        ...
        const formatted = {
            ...
            maxScore: quiz.questions.reduce((acc, q) => acc + (q.marks || 0), 0),
        };
...
exports.updateQuiz = async (req, res) => {
    try {
        ...
        console.error("Updated quiz error:", error);
...
exports.updateExam = async (req, res) => {
    try {
        ...
        console.error("Updated exam error:", error);
...
exports.getExamById = async (req, res) => {
    try {
        ...
        const formatted = {
            ...
            maxScore: exam.questions.reduce((acc, q) => acc + (q.marks || 0), 0),
        };
...
exports.getAllAssessmentItems = async (req, res) => {
    try {
        ...
        console.log(JSON.stringify(assessmentItems, null, 2));
        ...
        console.error("Get asssessment items error:", error);
...
exports.getAssessmentItemById = async (req, res) => {
    try {
        ...
        const formatted = {
            ...
            createdById: assessmentItem.created_by?._id,
            teacherName,
...
exports.updateAssessmentItem = async (req, res) => {
    try {
        ...
        assessmentItem.studentId = studentId || assessmentItem.studentId;
        assessmentItem.subjectId = subjectId || assessmentItem.subjectId;
        assessmentItem.klassId = klassId || assessmentItem.klassId;
        assessmentItem.sessionId = sessionId || assessmentItem.sessionId;
        assessmentItem.termId = termId || assessmentItem.termId;
        assessmentItem.type = type || assessmentItem.type;
        assessmentItem.title = title || assessmentItem.title;
        assessmentItem.score = score !== undefined ? score : assessmentItem.score;
        assessmentItem.maxScore = maxScore !== undefined ? maxScore : assessmentItem.maxScore;
        ...
        console.error("Update assessment item error:", error);
...
exports.getAssessmentItemFormData = async (req, res) => {
    try {
        ...
        const [
            ...
            Student.find().populate("userId", "title surname middlename lastname"),
        ]);

        const formatName = (u) => `${u?.title || ""} ${u?.surname || ""} ${u?.middlename || ""} ${u?.lastname || ""}`.trim();
        ...
        console.error("Get assessment item form data error:", error);
...
exports.createAssessmentItem = async (req, res) => {
    try {
        ...
        if (!studentId || !subjectId || !klassId || !sessionId || !termId || !type || !title || score === undefined || maxScore === undefined || !created_by) {
            return res.status(400).json({
                success: false,
                message: "Required fields missing",
            });
        ...
        console.error("Create assessment item error:", error);
...
exports.getAllAssessments = async (req, res) => {
    try {
        ...
        console.error("Get all asssessments error:", error);
...
exports.getAssessmentById = async (req, res) => {
    ...
    try {
        ...
        const assessment = await Assessment.findById(id)
            ...
            .populate("termId", "name")
            .populate({
                path: "quiz", select: "title type score maxScore",
            })
            .populate({
                path: "test",
